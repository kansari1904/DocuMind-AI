...
const FlashcardManager = ({ documentId }) => {
    ...
    const handleToggleStar = async (cardId) => {
        const selectedSetId = selectedSet?._id;

        // 🔁 Optimistic UI update
        const updatedSets = flashcardSets.map((set) => {
            if (set._id === selectedSetId) {
        ...
        setSelectedSet(updatedSets.find((set) => set._id === selectedSetId));
        ...
        try {
            ...
            // Re-fetch correct data from backend
            fetchFlashcardSets();
        }
    };
                ...
                <div className='flex flex-col items-center space-y-8'>
                    ...
                    <div className="flex items-center gap-6">
                        <button
                            ...
                            className="group flex items-center gap-2 px-5 h-11 bg-slate-200 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-slate-100"
                        >
                          <ChevronLeft
        ...
        <div className='bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl shadow-slate-200/50 p-8'>
            ...
            <Modal
                ...
                <button
                    ...
                    disabled = {deleting}
                            className= "px-5 h-11 bg-linear-to-r from-rose-500 to-red-500 hover:from-rose-600 hover:to-red-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-rose-500/25 active:scale-95 disabled:opacity-50  disabled:cursor-not-allowed active:disabled:scale-100"
                    >
                        {deleting ? (
